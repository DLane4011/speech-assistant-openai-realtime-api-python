...
# --- Reverting to the proven, stable voice model ---
VOICE_ID = "alloy" 
...
async def media_stream(websocket: WebSocket, lang: str):
    await websocket.acceptimport aiohttp
                        ...
                        async for raw in websocket.iter_text():
                            ...
                            if event == "start":
                                ...
                                if has_received_media:
                                    ...
                                    await ai_websocket.send_str(json.dumps({"type": "input_audio_buffer.()
    print(f"WebSocket connection accepted for language: {lang}")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.ws_connect(
                ...
                stream_sid = None
                has_received_media = False
                # --- FIX FOR CHOPPY AUDIO: A lightweight buffer ---
                audio_buffer = ""
                        ...
                        async for msg in ai_websocket:
                            if msg.type == aiohttp.WSMsgType.TEXT:
                                ...
                                if data.get("type") == "response.audio.delta" and "delta" in data:
                                    ...
                                    # Send audio in 80ms chunks to ensure smooth playback
...
async def setup_session(ai_ws, lang: str):
    ...
    prompt = (
        ...
        "when it occurred, where it took place, and if there is_to_send = audio_buffer[:640]
                                ...
                                print(f"OpenAI WebSocket error: { any evidence. "
...
async def send_greeting(ai_ws, lang: str):
    ...
    await ai_ws.msg}")
                        ...
                        print("OpenAI connection closed.")
                        # Ensure any last bit of audio is sent if the connection closes
                        if audio_buffer:
                            await websocket.send_json({"event": "media", "streamSid": stream_sid, "media": {"payload": audio_buffer}})
                            audio_buffer = ""
        ...
        print("Media stream finishedsend_str(json.dumps({
